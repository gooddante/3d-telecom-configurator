...
export async function loadCatalogue() {
  ...
  try {
    const response = await fetch('./public/assets/models.json');
    if (!response.ok) {
    ...
    if (category.products) {
      ...
      if (product) {
        return {
          ...
          model_url: product.filename ? `./public/assets/${product.filename}` : null
        };
      }
    }
...
export function getModelPath(product) {
  ...
  return `./public/assets/${product.filename}`;
}
